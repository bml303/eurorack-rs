//! `elements/dsp/fx/reverb.h` -- the Griesinger/Dattorro reverb (4 input
//! all-passes, then a 2x 2AP+delay loop), 16-bit ring buffer, modulated taps.
//! Applied to the stereo output as the "space" macro is turned up.
...
impl Reverb {
    ...
    pub fn process(&mut self, left: &mut [f32], right: &mut [f32]) {
        ...
        for (l, r) in left.iter_mut().zip(right.iter_mut()) {
            ...
            c.interpolate_lfo(BASES[AP1], 10.0, 0, 80.0, 1.0);
            c.write_line(BASES[AP1], LENGTHS[AP1], 100, 0.0);

            c.read_scaled(*l + *r, gain);
            ...
            // Main reverb loop.
            c.load(apout);
            c.interpolate_lfo(BASES[DEL2], 6211.0, 1, 100.0, krt);
            c.lp(&mut lp_1, klp);
