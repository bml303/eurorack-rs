//! `elements/dsp/string.{h,cc}` -- a single waveguide string (Karplus-Strong
//! comb with a FIR loop-damping filter, an IIR loss filter, a DC blocker and an
//! optional dispersion all-pass / curved-bridge non-linearity). Elements' STRING
//! and STRINGS resonator models run one and five of these respectively.
...
/// `elements::DampingFilter` -- the 3-tap FIR in the string's feedback loop,
/// with per-sample ramped `damping` and `brightness`.
...
impl String {
    ...
    /// `set_frequency(frequency, coefficient)` -- one-pole smoothed setter.
    #[inline]
    pub fn set_frequency_smoothed(&mut self, frequency: f32, coefficient: f32) {
        self.frequency += coefficient * (frequency - self.frequency);
    }

    #[inline]
    pub fn set_dispersion(&mut self, dispersion: f32) {
        let dispersion = if dispersion < 0.24 {
            (dispersion - 0.24) * 4.166
        } else if dispersion > 0.26 {
            (dispersion - 0.26) * 1.35135
        } else {
            0.0
        };
    ...
    fn process_internal<const DISPERSION: bool>(
        ...
        // Corner case (f0 < 11.7 Hz): not enough delay time, so play the lowest
        // note and upsample on the fly with a crude linear interpolator.
        ...
        for i in 0..size {
            ...
            if self.src_phase > 1.0 {
                ...
                s += input[i]; // f0 < 11.7 Hz bitcrushes the input -- as in the C.
...
/// `Interpolate(table, index, 1.0f)` -- the C helper with `size == 1`, i.e.
/// `index` is used directly (not scaled) as a fractional table position.
#[inline]
