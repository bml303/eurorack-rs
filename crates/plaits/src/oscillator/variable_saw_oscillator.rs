//! `plaits/dsp/oscillator/variable_saw_oscillator.h` -- saw with a variable
//! slope or notch (morphs toward a triangle).
...
impl VariableSawOscillator {
    ...
    pub fn render(&mut self, mut frequency: f32, mut pw: f32, waveshape: f32, out: &mut [f32]) {
        ...
        for o in out.iter_mut() {
            ...
            if !self.high && self.phase >= pw {
                ...
                let notch = (NOTCH_DEPTH + 1.0 - pw) * notch_amount;
            ...
            *o = (2.0 * this_sample - 1.0) / (1.0 + NOTCH_DEPTH);
        }
    ...
    fn compute_naive_sample(
        ...
        let notch_saw = if phase < pw { phase } else { 1.0 + NOTCH_DEPTH };
