//! `plaits/dsp/oscillator/variable_shape_oscillator.h` -- continuously
//! variable triangle -> saw -> square, with an optional hard-sync'ing master
//! phase and an optional phase-output mode (for phase distortion / FM).
...
impl VariableShapeOscillator {
    ...
    /// `Render(frequency, pw, waveshape, out, size)` -- no sync, no phase out.
    pub fn render(&mut self, frequency: f32, pw: f32, waveshape: f32, out: &mut [f32]) {
        self.render_full(false, false, 0.0, frequency, pw, waveshape, 0.0, out);
    }

    /// `Render(master_frequency, frequency, pw, waveshape, out, size)` -- hard sync.
    pub fn render_synced(
        ...
        self.render_full(true, false, master_frequency, frequency, pw, waveshape, 0.0, out);
    }

    /// `Render<enable_sync, output_phase>(...)` -- the fully general form.
    /// When `output_phase` is set, `out` receives the (possibly sync'ed,
    /// asymmetric) master phase plus `phase_modulation_amount * this_sample`,
    /// instead of the waveform itself -- used by the phase-distortion engine.
    ...
    pub fn render_full(
        ...
        let mut waveshape_modulation = ParameterInterpolator::new(&mut self.waveshape, waveshape, size);
        let mut phase_modulation =
            ParameterInterpolator::new(&mut self.phase_modulation, phase_modulation_amount, size);
        ...
        for o in out.iter_mut() {
            ...
            while transition_during_reset || !reset {
                if !self.high {
                    ...
                    let triangle_step = (slope_up + slope_down) * slave_frequency * triangle_amount;
            ...
            if output_phase {
                ...
                if enable_sync {
                    // Prevent discontinuities when the phase wraps around.
                    let w = 4.0 * (1.0 - self.master_phase) * self.master_phase;
                    this_sample *= w * (2.0 - w);

                    let p2 = phasor * phasor;
                    phasor += (p2 * p2 - phasor) * (pw - 0.5).abs() * 2.0;
                }
                *o = phasor + phase_modulation.next() * this_sample;
            } else {
    ...
    #[inline]
    pub fn set_master_phase(&mut self, master_phase: f32) {
