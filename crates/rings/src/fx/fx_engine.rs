//! `rings/dsp/fx/fx_engine.h` -- the tiny "delay-memory + accumulator"
//! virtual machine the Rings reverb, chorus and ensemble are written against
//! (Dattorro-style), a direct sibling of the clouds one.
//!
...
//! computes with [`bases`], and [`Context`] is a short-lived borrow of the ring
//! buffer. Sample storage format (16- / 32-bit) is a generic [`Format`]
//! parameter, preserved because the reverb's 16-bit quantisation is audible.
...
impl<F: Format, const SIZE: usize> FxEngine<F, SIZE> {
    ...
    /// `Init` (re-`Init`) -- clear the ring buffer.
...
impl<F: Format, const SIZE: usize> Context<'_, F, SIZE> {
    ...
    pub fn interpolate(&mut self, base: usize, offset: f32, scale: f32) {
        let offset_integral = offset as usize;
        let offset_fractional = offset - offset_integral as f32;
        let i0 = (self.write_ptr + offset_integral + base) & Self::MASK;
        let i1 = (self.write_ptr + offset_integral + base + 1) & Self::MASK;
