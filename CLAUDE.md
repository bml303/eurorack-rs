...
`mi-plaits`, `mi-clouds` and `mi-elements` (floating-point) plus `mi-edges`
(fixed-point, AVR) are ported; the other 11 module crates are scaffolds
(`Cargo.toml` + `lib.rs` + `PORTING.md` inventory). Read
